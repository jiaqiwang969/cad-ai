...
DEFAULT_PRODUCT_URL = "https://www.traceparts.cn/en/product/fibrotubi-orchidea-brick-wall-single-opening-sp105?CatalogPath=TRACEPARTS%3ATP12002018003001&Product=90-04092023-049404"
...
def extract_all_product_specifications(driver, product_id=None):
    ...
    try:
        ...
        # 🔧 FIX: 删除弹窗处理逻辑 - 显著提高速度，避免卡顿
        # 经测试，弹窗处理可能导致卡顿，且在很多情况下不是必需的
        print("⚡ 跳过弹窗处理，直接进行数据提取（性能优化）")
...
def main():
    ...
    try:
        ...
        # 🔧 FIX: 删除弹窗处理 - 显著提高速度，避免卡顿
        # close_disclaimer_popup(driver)  # 注释掉，经测试不是必需的
